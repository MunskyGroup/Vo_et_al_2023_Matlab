...
%% Make Initial Fit Figures (Figs 8B. Left 4 Columns)
close all
fitResults = makeFitFigures(vars,[FN,'_0_300']);
fitMLE = sum(fitResults.fitLikelihoods(:,[1,3]),2);
fitMLEs = fitMLE([1,3,4,5])
BICs = -2*fitMLE([1,3,4,5])+4*log(135+62)

%% Make Final Fit Figures (Not shown in paper, but may be useful)
close all
makeFitFigures(vars,[FN,'_0_18_300']);

%% Make Initial MH Scatter Plots (Figs 8B. Right 2 Columns)
close all
...
predMLEs = predMLE([1,3,4,5,7,8,9])
...
n = size(FIMR.fimResultsOne,1);
m = length(pars);
detFIM_Total_Best_0 = zeros(n,1);
sigsFIM_Total_Best_0 = zeros(n,m);
detFIM_Total_Best_0_18 = zeros(n,1);
sigsFIM_Total_Best_0_18 = zeros(n,m);
detFIM_Total_Best_0_300 = zeros(n,1);
sigsFIM_Total_Best_0_300 = zeros(n,m);
detFIM_Total_Best_0_18_300 = zeros(n,1);
sigsFIM_Total_Best_0_18_300 = zeros(n,m);
for i=1:n
...
Model = load([FN,'_0_18_300'],'ModelZero');
x = log10([Model.ModelZero{1}.parameters{2:end,2}]);
npar = length(x);
for k = 2:3:14
    figure(k)
    for i=1:npar-1
...
for i=1:4
    for j=1:4
        Atxt = strrep(Atxt,[va(i),'1',num2str(j)],num2str(log10(Model.ModelZero{pdo(j)}.parameters{vn(i),2}),'%0.3f'));

        smplDone = Model.chainResults{pdo(j)}.mhSamples(:,:);
        valDone = Model.chainResults{pdo(j)}.mhValue;
        smplDone = smplDone(valDone~=0,:);
        covMH = cov(smplDone/log(10));
        Atxt = strrep(Atxt,[va(i),'2',num2str(j)],num2str(sqrt(covMH(i,i)),'%0.3f'));

        covFIM = (Model.FIMZeroLog{pdo(j)}+diag([2 1 1 1]*log(10))^(-1))^(-1);
